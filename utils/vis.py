            ...
            if draw_dots:
                for lm_id in range(0,len(lm)):
                    if landmarks_to_draw is None or lm_id in landmarks_to_draw or len(lm) != 68:
                        ...
                        #     lm_color = (1,0,0)
                        cv2.circle(disp, tuple(lm[lm_id].astype(int)), radius=radius, color=lm_color, thickness=-1, lineType=cv2.LINE_AA)
...
def draw_results(X_resized, X_recon, levels_z=None,
    ...
    try:
        ...
        disp_X_recon = add_error_to_images(disp_X_recon, lm_errs, loc='br', format_string='{:>5.2f}', vmax=15,
